...
import { gameOpponent, setGameState, gameConnection, gameContract } from '../store.ts';
...
import { TurnType, TURNS } from '../types.ts';
...
export const deployContract = async (hostAddress: string, turn: TurnType) => {
  ...
  const salt = toHex(randomValues) as unknown as bigint;
  const hashWithSalt = encodePacked(['uint8', 'uint256'], [TURNS.indexOf(turn) + 1, salt]);
